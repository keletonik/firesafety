...
describe("Recurring Detection", () => {
  ...
  it("should detect fortnightly recurring payments", () => {
    const txns = [
      makeTxn({ merchant: "cleaner", txnDate: "2025-01-01", amount: -80 }),
      makeTxn({ merchant: "cleaner", txnDate: "2025-01-15", amount: -80 }),
      makeTxn({ merchant: "cleaner", txnDate: "2025-01-29", amount: -80 }),
      makeTxn({ merchant: "cleaner", txnDate: "2025-02-12", amount: -80 }),
    ];

    const result = detectRecurring(txns);
    const cleaner = result.find((r) => r.merchant === "cleaner");
    expect(cleaner).toBeDefined();
    expect(cleaner!.cadence).toBe("fortnightly");
  });

  it("should detect quarterly recurring payments", () => {
    const txns = [
      makeTxn({ merchant: "insurance co", txnDate: "2025-01-01", amount: -500 }),
      makeTxn({ merchant: "insurance co", txnDate: "2025-04-01", amount: -500 }),
      makeTxn({ merchant: "insurance co", txnDate: "2025-07-01", amount: -500 }),
    ];

    const result = detectRecurring(txns);
    const insurance = result.find((r) => r.merchant === "insurance co");
    expect(insurance).toBeDefined();
    expect(insurance!.cadence).toBe("quarterly");
  });
