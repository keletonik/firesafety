  ...
  describe("computeCashflow", () => {
    ...
    it("should skip zero-amount transactions", () => {
      const txns = [
        ...
        makeTxn({ amount: 0, txnDate: "2025-01-02" }),
        makeTxn({ amount: -200, txnDate: "2025-01-03" }),
      ];
    ...
    it("should avoid floating-point drift with many small amounts", () => {
      // 0.1 + 0.2 !== 0.3 in naive float math
      const txns = Array.from({ length: 100 }, (_, i) =>
        makeTxn({ amount: -0.1, txnDate: `2025-01-${String(i % 28 + 1).padStart(2, "0")}` })
      );

      const result = computeCashflow(txns);
      // With integer-cent arithmetic, this should be exactly 10.00
